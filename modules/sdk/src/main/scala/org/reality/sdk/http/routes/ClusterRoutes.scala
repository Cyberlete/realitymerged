package org.reality.sdk.http.routes
...
final case class ClusterRoutes[F[_]: Async](
  ...
  private val p2pPublic: HttpRoutes[F] = HttpRoutes.of[F] {
    case req @ POST -> Root / "join" =>
      req.decodeR[JoinRequest] { joinRequest: JoinRequest =>
        logger.error(s"joinRequest $joinRequest ").void
        req.remoteAddr
          .flatMap(_.asIpv4)
          .map(_.toString)
          .flatMap(Host.fromString)
          .fold(BadRequest())((host: Host) =>
            joining
              ...
              .recoverWith {
                ...
                case a @ _                             => InternalServerError(s"Unknown error. a - ${a.toString}")
              }
          )
