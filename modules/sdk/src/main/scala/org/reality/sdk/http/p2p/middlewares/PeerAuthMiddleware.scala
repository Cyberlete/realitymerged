package org.reality.sdk.http.p2p.middleware
...
import org.reality.schema.cluster.TokenValid
...
import org.reality.sdk.http.p2p.headers.{`X-Id`, `X-Session-Token`}
...
object PeerAuthMiddleware {
  ...
  def responseSignerMiddleware[F[_]: Async: SecurityProvider](
    ...
    Kleisli { req: Request[F] =>
      for {
        ...
        signedResponse <- new Http4sResponseSigner[F](getGenerator(privateKey), new RealityHttpCryptoConfig {})
          .sign(resWithHeader)
          .attemptT
          .toOption
  ...
  def responseVerifierMiddleware[F[_]: Async: SecurityProvider](peerId: PeerId)(client: Client[F]): Client[F] =
    Client { (req: Request[F]) =>
      val verifier = peerId.value.toPublicKey.map { publicKey =>
        new Http4sResponseVerifier[F](getVerifier(publicKey), new RealityHttpCryptoConfig {})
      }
  ...
  def requestSignerMiddleware[F[_]: Async: SecurityProvider](
    ...
    val signer = new Http4sRequestSigner(getGenerator(privateKey), new RealityHttpCryptoConfig {})
  ...
  def requestVerifierMiddleware[F[_]: Async: SecurityProvider](http: HttpRoutes[F]): HttpRoutes[F] =
    Kleisli { req: Request[F] =>
      val verify: OptionT[F, Response[F]] = for {
        ...
        verifier = new Http4sRequestVerifier[F](crypto, new RealityHttpCryptoConfig {})
  ...
  trait RealityHttpCryptoConfig extends HttpCryptoConfig {
