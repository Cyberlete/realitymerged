package org.reality.sdk.domain.cluster.programs
...
import org.reality.sdk.config.AppEnvironment.Dev
...
import org.reality.sdk.http.p2p.clients.SignClient
...
object Joining {
  ...
  def make[F[_]: Async: GenUUID: SecurityProvider](
...
sealed abstract class Joining[F[_]: Async: GenUUID: SecurityProvider] private (
  environment: AppEnvironment,
  nodeStorage: NodeStorage[F],
  clusterStorage: ClusterStorage[F],
  signClient: SignClient[F],
  ...
  def joinRequest(hasCollateral: PeerId => F[Boolean])(joinRequest: JoinRequest, remoteAddress: Host): F[Unit] = {
    for {
      inC <- nodeStorage.getNodeState.map(NodeState.inCluster)
      _ <- logger.error(s"joinRequest inCluster - ${inC}")
      ...
      _ <- logger.error(s"beginning twoWayHandshake withPeer $withPeer and remoteAddress $remoteAddress")

      _ <- twoWayHandshake(withPeer, remoteAddress.some, skipJoinRequest = true)
      _ <- logger.error(s"after twoWayHandshake withPeer")
  ...
  private def twoWayHandshake(
    ...
    for {
      ...
      _ <- logger.error(s"twoWayHandshake remoteAddress - $remoteAddress")
      ...
      signedSignRequest: Signed[SignRequest] <- signClient.sign(signRequest).run(withPeer)
      _ <- logger.error(s"twoWayHandshake isValid - $signedSignRequest")
      ...
      isValid <- verifySignRequest(signRequest, signedSignRequest, PeerId._Id.get(withPeer.id))

      _ <- logger.error(s"twoWayHandshake isValid - $isValid")
      _ <-
        if (skipJoinRequest) {
          Applicative[F].unit
        } else {
