    ...
    new BlockAcceptanceManager[F] {
      ...
      def acceptBlocksIteratively(
        ...
        def go(
          ...
          toProcess: List[(Signed[NETBlock], TxChains)]
        ...
        blocks.sorted
          .foldLeftM((List.empty[(Signed[NETBlock], TxChains)], List.empty[(Signed[NETBlock], ValidationFailed)])) { (acc, block) =>
            acc match {
      ...
      private def logAcceptedBlock(tuple: (Signed[NETBlock], NonNegLong)): F[Unit] = {
      ...
      private def logNotAcceptedBlock(tuple: (Signed[NETBlock], BlockNotAcceptedReason)): F[Unit] = {
