...
import org.reality.schema.balance.{Amount, Balance, BalanceArithmeticError}
import org.reality.schema.transaction.{StandardTransaction, Transaction, TransactionReference}
...
object BlockAcceptanceLogic {
  ...
  def make[F[_]: Async: SecurityProvider]: BlockAcceptanceLogic[F] = new BlockAcceptanceLogic[F] {
    ...
    private def processBalances(
      ...
      val sortedTxs = block.transactions.filter(_.value.isInstanceOf[StandardTransaction]).toList
