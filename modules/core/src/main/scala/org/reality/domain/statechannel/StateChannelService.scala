...
import org.reality.sdk.domain.statechannel.StateChannelValidator.StateChannelValidationError
...
object StateChannelService {
  ...
  def make[F[_]: Async](mkNetCell: L0Cell.Mk[F], stateChannelValidator: StateChannelValidator[F]): StateChannelService[F] =
    new StateChannelService[F] {
      ...
      def process(stateChannelOutput: StateChannelOutput): F[Either[NonEmptyList[StateChannelValidationError], Unit]] =
        stateChannelValidator.validate(stateChannelOutput).flatMap {
          case Valid(_) =>
            mkNetCell(L0CellInput.HandleStateChannelSnapshot(stateChannelOutput))
              .run()
