...
import org.reality.generators._
...
import org.reality.security.generators._
...
object generators {
  ...
  val addressGen: Gen[Address] =
    ...
    } yield Address(refineV[NETAddressRefined].unsafeFrom(s"NET$par$end"))

  implicit val addressArbitrary: Arbitrary[Address] = Arbitrary(addressGen)
  ...
  val transactionGen: Gen[Transaction] =
    ...
    } yield StandardTransaction(src, dst, txnAmount, txnFee, txnReference, txnSalt)
