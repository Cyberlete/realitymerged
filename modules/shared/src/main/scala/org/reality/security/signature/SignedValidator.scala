...
import org.reality.security.signature.SignedValidator.SignedValidationErrorOr
...
trait SignedValidator[F[_]] {
  ...
  def validateUniqueSigners[A](
    signed: Signed[A]
  ): SignedValidationErrorOr[Signed[A]]

  def validateMinSignatureCount[A](
    signed: Signed[A],
    minSignatureCount: PosInt
  ): SignedValidationErrorOr[Signed[A]]

  def isSignedExclusivelyBy[A](
    signed: Signed[A],
