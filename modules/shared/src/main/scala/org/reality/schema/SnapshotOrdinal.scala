...
object SnapshotOrdinal {
  def fromLong(value: Long): Option[SnapshotOrdinal] =
    NonNegLong.from(value).toOption.map(SnapshotOrdinal(_))

  def unsafeFromLong(value: Long): SnapshotOrdinal =
  ...
  implicit val keyIndex: KeyIndex[SnapshotOrdinal] = new KeyIndex[SnapshotOrdinal] {
    def index(a: SnapshotOrdinal): Long = a.value.value
  }
}
