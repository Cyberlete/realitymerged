...
import org.reality.combined.{CoCell, Portal, ProofGenerationErrorWasm, WasmError, WasmExecutorCoCell, ZKProofError, ZKWasmExecutor}
...
class MovementAnalysisExample extends Portal with ZKWasmMovementAnalysis {
  import CoCell._
  ...
  private def convertButtonsToFlags(event: MouseEvent): Int = {
    var flags = 0
  ...
  protected val wasmExecutor = new ZKWasmExecutor[IO]
