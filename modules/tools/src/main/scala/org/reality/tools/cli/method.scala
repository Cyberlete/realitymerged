package org.reality.tools.cli

import java.nio.file.Path

import cats.syntax.all._

import scala.concurrent.duration.FiniteDuration

import org.reality.ext.decline.WithOpts
import org.reality.ext.decline.decline.coercibleArgument

import com.monovore.decline.Opts
import com.monovore.decline.refined.refTypeArgument
import eu.timepit.refined.api.RefType.refinedRefType
import eu.timepit.refined.api.Refined
import eu.timepit.refined.auto._
import eu.timepit.refined.numeric.GreaterEqual
import eu.timepit.refined.refineV
import eu.timepit.refined.string.Url
import eu.timepit.refined.types.numeric._

object method {

  type IntGreaterEqual2 = Int Refined GreaterEqual[2]
  type UrlString = String Refined Url

  sealed trait CliMethod

  case class BasicOpts(
    baseUrl: UrlString,
    take: Option[PosLong],
    chunkSize: PosInt,
    delay: Option[FiniteDuration],
    retryAttempts: NonNegInt,
    verbose: Boolean,
    fee: NonNegLong
  )

  case class SendDeployAppTransactionBasicOpts(
    baseUrl: UrlString,
    appName: String,
    appVersion: String,
    appDescription: String,
    appDownloadUrl: UrlString,
    appDataPath: Path,
    fee: NonNegLong,
    destinationWalletPath: Path
  )

  case class SendStandardTransactionBasicOpts(
    baseUrl: UrlString,
    amount: PosLong,
    fee: NonNegLong,
    destinationWalletPath: Path
  )

  case class SendTransactionWalletOpts(
    walletPath: Path,
    alias: String,
    password: String
  )

  case class SendTransactionsCmd(
    basicOpts: BasicOpts,
    walletsOpts: WalletsOpts,
    verbose: Boolean
  ) extends CliMethod

  case class SendStateChannelSnapshotCmd(
    baseUrl: UrlString,
    verbose: Boolean
  ) extends CliMethod

  case class SendStandardTransactionCmd(
    standardTransactionBasicOpts: SendStandardTransactionBasicOpts,
    standardTransactionWalletOpts: SendTransactionWalletOpts
  ) extends CliMethod

  case class SendDeployAppTransactionCmd(
    deployAppBasicOpts: SendDeployAppTransactionBasicOpts,
    deployAppTransactionWalletOpts: SendTransactionWalletOpts
  ) extends CliMethod

  sealed trait WalletsOpts
  case class GeneratedWallets(count: IntGreaterEqual2, genesisPath: Path) extends WalletsOpts
  case class LoadedWallets(walletsPath: Path, alias: String, password: String) extends WalletsOpts

  object SendTransactionsCmd extends WithOpts[SendTransactionsCmd] {
    private val basicOpts = (
      Opts.argument[String](metavar = "baseUrl").map(withProtocol).mapValidated(refineV[Url](_).toValidatedNel),
      Opts.option[PosLong]("take", "Number of transactions. Infinite if unspecified.", "t").orNone,
      Opts.option[PosInt]("chunk", "Size of a chunk, default 1.", "c").withDefault(PosInt(1)),
      Opts.option[FiniteDuration]("delay", "Delay before sending each transaction.", "d").orNone,
      Opts
        .option[NonNegInt]("retryAttempts", "Number of retry attempts to send transaction, default 10.")
        .withDefault(NonNegInt(10)),
      Opts.flag("verbose", "Print individual transactions.", "v").map(_ => true).withDefault(false),
      Opts.option[NonNegLong]("fee", "Transaction fee, default 1.", "f").withDefault(NonNegLong(1L))
    ).mapN(BasicOpts.apply)

    private val generatedWallets = (
      Opts.option[IntGreaterEqual2]("generateWallets", "Number of wallets to generate, at least 2."),
      Opts.option[Path]("genesisPath", "Specifies where genesis should be stored.")
    ).mapN(GeneratedWallets)

    private val loadedWallets = (
      Opts.option[Path]("loadWallets", "Specifies where wallets (.p12 files) will be loaded from."),
      Opts.option[String]("alias", "Universal alias for all keys, default `alias`.").withDefault("alias"),
      Opts.option[String]("password", "Universal password for all keys, default `password`.").withDefault("password")
    ).mapN(LoadedWallets.apply)

    val opts: Opts[SendTransactionsCmd] = Opts.subcommand("send-transactions", "Send sample transactions") {
      (
        basicOpts,
        generatedWallets.orElse(loadedWallets)
      ).mapN((basicOpts, walletsOpts) => SendTransactionsCmd(basicOpts, walletsOpts, verbose = false))
    }
  }

  object SendStateChannelSnapshotCmd extends WithOpts[SendStateChannelSnapshotCmd] {

    val opts: Opts[SendStateChannelSnapshotCmd] =
      Opts.subcommand("send-state-channel-snapshot", "Send sample state-channel snapshot") {
        (
          Opts.argument[String](metavar = "baseUrl").map(withProtocol).mapValidated(refineV[Url](_).toValidatedNel),
          Opts.flag("verbose", "Display debug messages", "v").map(_ => true).withDefault(false)
        ).mapN(SendStateChannelSnapshotCmd.apply)
      }
  }

  object SendStandardTransactionCmd extends WithOpts[SendStandardTransactionCmd] {
    private val basicOpts = (
      Opts
        .option[String]("baseUrl", "The base URL to connect to")
        .map(withProtocol)
        .mapValidated(refineV[Url](_).toValidatedNel),
      Opts.option[PosLong]("amount", "Transaction amount, default 1.", "f"),
      Opts.option[NonNegLong]("fee", "Transaction fee, default 1.", "f").withDefault(NonNegLong(1L)),
      Opts.option[Path]("destinationWalletPath", "Destination wallet path")
    ).mapN(SendStandardTransactionBasicOpts.apply)

    private val walletOpts = (
      Opts.option[Path]("walletPath", "Path to the wallet file to use for the transaction"),
      Opts.option[String]("alias", "Alias for the wallet", "a").withDefault("alias"),
      Opts.option[String]("password", "Password for the wallet", "p").withDefault("password")
    ).mapN(SendTransactionWalletOpts)

    val opts: Opts[SendStandardTransactionCmd] =
      Opts.subcommand("send-standard-transaction", "Send standard transaction") {
        (basicOpts, walletOpts).mapN((b, s) => SendStandardTransactionCmd(b, s))
      }
  }

  object SendDeployAppTransactionCmd extends WithOpts[SendDeployAppTransactionCmd] {
    private val basicOpts = (
      Opts
        .option[String]("baseUrl", "The base URL to connect to")
        .map(withProtocol)
        .mapValidated(refineV[Url](_).toValidatedNel),
      Opts.option[String]("appName", "The name of the application").withDefault("ExampleApp"),
      Opts.option[String]("appVersion", "The version of the application").withDefault("1.0.0"),
      Opts.option[String]("appDescription", "The description of the application").withDefault("No description provided"),
      Opts
        .option[String]("appDownloadUrl", "The base URL to connect to")
        .map(withProtocol)
        .mapValidated(refineV[Url](_).toValidatedNel),
      Opts.option[Path]("appDataPath", "The path to the application's data file"),
      Opts.option[NonNegLong]("fee", "Transaction fee, default 1.", "f").withDefault(NonNegLong(1L)),
      Opts.option[Path]("destinationWalletPath", "Destination wallet path")
    ).mapN(SendDeployAppTransactionBasicOpts.apply)

    private val walletOpts = (
      Opts.option[Path]("walletPath", "Path to the wallet file to use for the transaction"),
      Opts.option[String]("alias", "Alias for the wallet", "a").withDefault("alias"),
      Opts.option[String]("password", "Password for the wallet", "p").withDefault("password")
    ).mapN(SendTransactionWalletOpts)

    val opts: Opts[SendDeployAppTransactionCmd] =
      Opts.subcommand("send-deploy-app-transaction", "Send deploy app transaction") {
        (basicOpts, walletOpts).mapN((b, s) => SendDeployAppTransactionCmd(b, s))
      }
  }

  val opts: Opts[CliMethod] = SendTransactionsCmd.opts
    .orElse(SendStateChannelSnapshotCmd.opts)
    .orElse(SendDeployAppTransactionCmd.opts)
    .orElse(SendStandardTransactionCmd.opts)

  private val defaultProtocol = "http://"

  private def withProtocol(url: String): String =
    if (url.matches("^[a-z]+://"))
      url
    else
      defaultProtocol + url
}
