...
import org.reality.keytool.cert.{DistinguishedName, SelfSignedCertificate}
import org.reality.security._
...
object KeyStoreUtils {
  ...
  private val peerIdHexName: String = ".peerId.hex"
  ...
  private[keytool] def exportPeerId[F[_]: Async: SecurityProvider](
    path: String,
    alias: String,
    storePassword: Array[Char],
    keyPassword: Array[Char]
    ...
    for {
      keyPair <- readKeyPairFromStore(path, alias, storePassword, keyPassword)
      peerId = keyPair.getPublic.toId.hex
      _ <- writer(path + peerIdHexName)
        .use(os =>
          Async[F].delay {
            os.write(peerId.coerce.getBytes(Charset.forName("UTF-8")))
          }
        )
    } yield peerId

  private[keytool] def generateMnemonic[F[_]: Async: SecurityProvider](
    path: String,
    alias: String,
    passphrase: String,
    ...
    for {
      seed <- Async[F].delay(BIP44.generateDeterministicSeed(passphrase))
      mnemonicString = seed.getMnemonicString
      _ <- Async[F].delay(println(s"${mnemonicString}:${seed.getCreationTimeSeconds}"))
      bip44 = new BIP44(mnemonicString)
      keyPair = bip44.getChildKeyPairOfDepth()
      _ <- writeKeyPairToStore(keyPair, path, alias, mnemonicString.toCharArray, distinguishedName, certificateValidityDays)
    } yield seed
