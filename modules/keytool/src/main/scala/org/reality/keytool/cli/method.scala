package org.reality.keytool.cli
...
object method {
  ...
  val distinguishedName = DistinguishedName(
    commonName = "realitynet.xyz",
    organization = "RealityNet"
  )
  ...
  object ExportPeerId extends WithOpts[ExportPeerId] {
    val opts = Opts.subcommand("peerId", "Gets PeerId from KeyStore") {
      (
        StorePath.opts,
        KeyAlias.opts,
        (StorePass.opts, KeyPass.opts).tupled
        ...
        case (storepath, alias, (storepass, keypass)) => ExportPeerId(storepath, alias, storepass, keypass)
      }
    }
  }

  case class GenerateMnemonic(
    keyStore: StorePath,
    alias: KeyAlias,
    passphrase: Passphrase = Passphrase(Secret("")),
    distinguishedName: DistinguishedName = distinguishedName,
    certificateValidityDays: Long = certificateValidityDays
  ) extends CliMethod

  object GenerateMnemonic extends WithOpts[GenerateMnemonic] {
    val opts = Opts.subcommand("generateMnemonic", "Generates Mnemonic") {
      (StorePath.opts, KeyAlias.opts).mapN(GenerateMnemonic.apply(_, _))
    }
  }
  ...
  val opts: Opts[CliMethod] =
    GenerateWallet.opts
      ...
      .orElse(ExportPeerId.opts)
      .orElse(GenerateMnemonic.opts)
}
