...
import BlockConsensusInput.Proposal
...
import org.reality.schema.transaction.{TransactionFee, TransactionReference}
...
object RoundDataSuite extends ResourceSuite with Checkers with TransactionGenerator {
  ...
  override type Res = (SecurityProvider[IO], KeyPair, KeyPair, Address, Address, TransactionValidator[IO])
  ...
  override def sharedResource: Resource[IO, Res] =
    SecurityProvider.forAsync[IO].flatMap { implicit sp =>
      ...
      } yield (sp, srcKey, dstKey, srcAddress, dstAddress, txValidator)
    }
  ...
  test(
    ...
    case (sp, srcKey, _, srcAddress, dstAddress, txValidator) =>
      ...
      for {
        ...
        expect.same(
          NETBlock(baseProposal.tips.value, txsA2.map(_.signed).toNes).some,
          result
        )
  }
  ...
  test("formBlock should pick transactions correctly from the pool of transactions from all facilitators") {
    case (sp, srcKey, _, srcAddress, dstAddress, txValidator) =>
      ...
      for {
        ...
        expect.same(
          NETBlock(baseProposal.tips.value, (txsA.map(_.signed) ++ txsA2.map(_.signed).toList).toNes).some,
          result
        )
  }
  ...
  test("formBlock should pick transactions correctly when concurrent transactions are proposed by different facilitators") {
    case (sp, srcKey, _, srcAddress, dstAddress, txValidator) =>
      ...
      for {
        ...
        expect.same(
          NETBlock(baseProposal.tips.value, (NonEmptyList.one(txsA.head.signed) ++ txsA2.map(_.signed).toList).toNes).some,
          result
        )
  }

  test("formBlock should discard transactions that are invalid") {
    case (sp, srcKey, dstKey, srcAddress, dstAddress, txValidator) =>
      implicit val securityProvider = sp
      ...
      for {
        ...
        expect.same(
          NETBlock(baseProposal.tips.value, NonEmptyList.one(txsA.head.signed).toNes).some,
          result
        )
  }
}
