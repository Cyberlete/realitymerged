...
import org.reality.dag.block.BlockValidator.BlockValidationParams
...
import org.reality.dag.l1.domain.transaction.{TransactionStorage, transactionLoggerName}
import org.reality.effects.GenUUID
import org.reality.ext.collection.MapRefUtils.MapRefOps
...
import org.reality.security.signature.Signed._
...
import BlockConsensusCell.{Algebra, Coalgebra}
import BlockConsensusInput._
import BlockConsensusOutput.{CleanedConsensuses, FinalBlock}
import CancellationReason._
import CoalgebraCommand._
import Validator.isReadyForBlockConsensus

class BlockConsensusCell[F[_]: Async: SecurityProvider: Random](
  data: Ω,
  ctx: BlockConsensusContext[F]
) extends Cell[F, StackF, CoalgebraCommand, Ω, Either[CellError, Ω]](
      data,
      scheme.hyloM(
        ...
        CoalgebraM[F, StackF, Ω] {
          ...
          case _ => Applicative[F].pure(Done(NoAction.asRight[CellError]))
      ...
      ),
      {
        ...
        case _                                              => InspectConsensuses
      }
    )

trait StateChannelCell {
  def mkCell[F[_]: Async: SecurityProvider: Random](
    ctx: BlockConsensusContext[F]
  ): Ω => Cell[F, StackF, Ω, Ω, Either[CellError, Ω]]
}
object BlockConsensusCell extends StateChannelCell {

  def mkCell[F[_]: Async: SecurityProvider: Random](
    ctx: BlockConsensusContext[F]
  ): Ω => Cell[F, StackF, Ω, Ω, Either[CellError, Ω]] = (data: Ω) =>
  ...
  object Algebra {
    ...
    private def sendOwnProposal[F[_]: Async: SecurityProvider](
      ownProposal: Proposal,
      peers: Set[Peer],
      context: BlockConsensusContext[F]
    ): F[Unit] =
      for {
        signedProposal <- Signed.forAsyncJson[F, PeerBlockConsensusInput](ownProposal, context.keyPair)
        _ <- broadcast(signedProposal, peers, context.blockConsensusClient)
      } yield ()

    def persistInitialOwnRoundData[F[_]: Async: SecurityProvider](
      roundData: RoundData,
      ctx: BlockConsensusContext[F]
    ...
    def persistInitialPeerRoundData[F[_]: Async: SecurityProvider](
      roundData: RoundData,
    ...
    private def sendBlockProposal[F[_]: Async: SecurityProvider](
      signedBlock: Signed[NETBlock],
      roundData: RoundData,
      ctx: BlockConsensusContext[F]
    ): F[Either[CellError, Ω]] =
      for {
        ...
        signedBlockSignatureProposal <- Signed
          .forAsyncJson[F, PeerBlockConsensusInput](blockSignatureProposal, ctx.keyPair)
    ...
    private def processValidBlock[F[_]: Async: SecurityProvider](
      proposal: Proposal,
      signedBlock: Signed[NETBlock],
      ctx: BlockConsensusContext[F]
    ): F[Either[CellError, Ω]] =
    ...
    def persistProposal[F[_]: Async: SecurityProvider](
      proposal: Proposal,
      ctx: BlockConsensusContext[F]
      ...
      .flatMap {
        case Some(roundData) if gotAllProposals(roundData) =>
          roundData.formBlock(ctx.transactionValidator).flatMap {
            case Some(block) =>
              Signed.forAsyncJson(block, ctx.keyPair).flatMap { signedBlock =>
                ctx.blockValidator
    ...
    def persistBlockSignatureProposal[F[_]: Async: SecurityProvider](
      blockSignatureProposal: BlockSignatureProposal,
      ctx: BlockConsensusContext[F]
    ): F[Either[CellError, Ω]] =
    ...
    def informAboutInabilityToParticipate[F[_]: Async: SecurityProvider](
      proposal: Proposal,
      ...
      for {
        ...
        signedCancellationMessage <- Signed
          .forAsyncJson[F, PeerBlockConsensusInput](cancellation, ctx.keyPair)
    ...
    def processCancellation[F[_]: Async: SecurityProvider](
      cancellation: CancelledBlockCreationRound,
      ctx: BlockConsensusContext[F]
    ): F[Either[CellError, Ω]] =
      for {
        ...
        _ <- maybeUpdated match {
          case Some((roundData, Some(myCancellation))) =>
            for {
              signedCancellationMessage <- Signed
                .forAsyncJson[F, PeerBlockConsensusInput](myCancellation, ctx.keyPair)
  ...
  object Coalgebra {
    ...
    def startOwnRound[F[_]: Async: Random](ctx: BlockConsensusContext[F]): F[StackF[Ω]] =
      for {
    ...
    def inspectConsensuses[F[_]: Async](ctx: BlockConsensusContext[F]): F[StackF[Ω]] =
      for {
