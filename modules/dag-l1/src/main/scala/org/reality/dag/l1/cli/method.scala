package org.reality.dag.l1.cli

import cats.syntax.contravariantSemigroupal._

import scala.concurrent.duration.{DurationDouble, DurationInt}

import org.reality.cli.env.{KeyAlias, Password, StorePath}
import org.reality.dag.l1.config.types.{L1AppConfig, L1DBConfig}
import org.reality.dag.l1.domain.consensus.block.config.ConsensusConfig
import org.reality.ext.decline.decline._
import org.reality.schema.balance.Amount
import org.reality.schema.node.NodeState
import org.reality.schema.peer.L0Peer
import org.reality.sdk.cli.{CliMethod, CollateralAmountOpts, L0PeerOpts}
import org.reality.sdk.config.AppEnvironment
import org.reality.sdk.config.types.{SnapshotConfig, _}

import com.monovore.decline.Opts
import eu.timepit.refined.auto.autoRefineV
import fs2.io.file.Path
trait Run extends CliMethod {
  val dbConfig: DBConfig

  val stateAfterJoining: NodeState = NodeState.Ready

  override val appConfig: L1AppConfig = L1AppConfig(
    environment = environment,
    http = httpConfig,
    db = dbConfig,
    gossip = GossipConfig(
      storage = RumorStorageConfig(
        peerRumorsCapacity = 50L,
        activeCommonRumorsCapacity = 20L,
        seenCommonRumorsCapacity = 50L
      ),
      daemon = GossipDaemonConfig(
        peerRound = GossipRoundConfig(
          fanout = 1,
          interval = 0.2.seconds,
          maxConcurrentRounds = 4
        ),
        commonRound = GossipRoundConfig(
          fanout = 1,
          interval = 0.5.seconds,
          maxConcurrentRounds = 2
        )
      )
    ),
    consensus = ConsensusConfig(
      peersCount = 2,
      tipsCount = 2,
      timeout = 45.seconds,
      pullTxsCount = 100L
    ),
    healthCheck = healthCheckConfig(false),
    collateral = collateralConfig(environment, collateralAmount)
  )
}
object method {

  case class RunInitialValidator(
    keyStore: StorePath,
    alias: KeyAlias,
    password: Password,
    environment: AppEnvironment,
    httpConfig: HttpConfig,
    dbConfig: L1DBConfig,
    l0Peer: L0Peer,
    seedlistPath: Option[Path],
    collateralAmount: Option[Amount],
    val snapshotConfig: SnapshotConfig = SnapshotConfig(
      org.reality.sdk.config.types
        .ConsensusConfig(10.seconds, 10.seconds, 10.seconds, ObservationConfig(10.seconds, 10.seconds, 1L), SelectPeersConfig(4, 4)),
      globalSnapshotPath = Path("data/snapshot"),
      inMemoryCapacity = 10L
    )
  ) extends Run

  object RunInitialValidator {
    val seedlistPathOpts: Opts[Option[Path]] = Opts.option[Path]("seedlist", "").orNone

    val opts = Opts.subcommand("run-initial-validator", "Run initial validator mode") {
      (
        StorePath.opts,
        KeyAlias.opts,
        Password.opts,
        AppEnvironment.opts,
        http.opts,
        db.opts,
        L0PeerOpts.opts,
        seedlistPathOpts,
        CollateralAmountOpts.opts
      ).mapN(RunInitialValidator(_, _, _, _, _, _, _, _, _))
    }
  }

  case class RunValidator(
    keyStore: StorePath,
    alias: KeyAlias,
    password: Password,
    environment: AppEnvironment,
    httpConfig: HttpConfig,
    dbConfig: L1DBConfig,
    l0Peer: L0Peer,
    seedlistPath: Option[Path],
    collateralAmount: Option[Amount],
    val snapshotConfig: SnapshotConfig = SnapshotConfig(
      org.reality.sdk.config.types
        .ConsensusConfig(10.seconds, 10.seconds, 10.seconds, ObservationConfig(10.seconds, 10.seconds, 1L), SelectPeersConfig(4, 4)),
      globalSnapshotPath = Path("data/snapshot"),
      inMemoryCapacity = 10L
    )
  ) extends Run

  object RunValidator {
    val seedlistPathOpts: Opts[Option[Path]] = Opts.option[Path]("seedlist", "").orNone

    val opts = Opts.subcommand("run-validator", "Run validator mode") {
      (
        StorePath.opts,
        KeyAlias.opts,
        Password.opts,
        AppEnvironment.opts,
        http.opts,
        db.opts,
        L0PeerOpts.opts,
        seedlistPathOpts,
        CollateralAmountOpts.opts
      ).mapN(RunValidator(_, _, _, _, _, _, _, _, _))
    }
  }

  val opts: Opts[Run] =
    RunInitialValidator.opts.orElse(RunValidator.opts)
}
