...
//  def getSwapTxs(source: Address): F[TransactionReference] = waitingTransactions(source).get.getOrElse(Transaction.empty))
...
object TransactionStorage {
  ...
  def make[F[_]: Async]: F[TransactionStorage[F]] =
    for {
