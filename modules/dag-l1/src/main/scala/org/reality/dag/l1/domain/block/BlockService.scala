...
import org.reality.schema.{BlockReference, transaction}
...
trait BlockService[F[_]] {
  def accept(signedBlock: Signed[NETBlock]): F[Unit]
}
...
object BlockService {
  ...
  def make[F[_]: Async](
    ...
    new BlockService[F] {
      ...
      private def processAcceptanceSuccess(hashedBlock: Hashed[NETBlock])(contextUpdate: BlockAcceptanceContextUpdate): F[Unit] = for {
        ...
        isDependent = (block: Signed[NETBlock]) => BlockRelations.dependsOn(hashedBlock)(block)
        _ <- blockStorage.restoreDependent(isDependent)
      } yield ()

      private def processAcceptanceError(hashedBlock: Hashed[NETBlock])(reason: BlockNotAcceptedReason): F[BlockAcceptanceError] =
        blockStorage
