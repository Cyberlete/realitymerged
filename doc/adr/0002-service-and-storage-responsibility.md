...
For example, where NET Blocks should be stored, where should be accepted, where
...
2. We will add an extra logic to `Storage` (see `BlockStorage`) due to required
   local state changes.
3. NET Block validation will be a separate service (stateless) that will have a
   single responsibility and can depend on `BlockStorage` to make required
   validation.
4. NET Block acceptance will take place in `BlockService` that will aggregate
